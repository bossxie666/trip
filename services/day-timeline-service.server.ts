...
export async function getDayTimeline(tripId: string, dayId: string): Promise<DayTimelineEntry[]> {
  ...
  if (!context.date) throw new Error("TIMELINE_CONTEXT_INCOMPLETE");
  ...
  return buildDayTimeline({ dayDate: context.date, timezone, bookings, items, placements: positions.map((position) => ({ sourceType: position.sourceType, sourceId: position.sourceId, anchorType: position.anchorType, sortOrder: position.sortOrder })) });
}
