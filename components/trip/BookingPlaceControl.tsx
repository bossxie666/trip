 ...
 */
export function BookingPlaceControl({ slug, bookingId, slot, cities, currentPlace, label = "修改地点", defaultCityId }: { slug: string; bookingId: string; slot: Slot; cities: City[]; currentPlace: Place; label?: string; defaultCityId?: string | null }) {
  ...
  async function bind(providerPlaceId: string) {
    ...
    try {
      ...
      const field = slot === "origin" ? "originPlaceId" : slot === "destination" ? "destinationPlaceId" : "placeId";
      const bookingResponse = await fetch(`/api/trips/${encodeURIComponent(slug)}/budget/bookings/${encodeURIComponent(bookingId)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ [field]: placePayload.place.id }) });
  ...
  return <details className="booking-place-control" open={open} onToggle={(event) => setOpen((event.currentTarget as HTMLDetailsElement).open)}><summary>{label}</summary><div className="booking-place-editor"><label>城市<select value={cityId} onChange={(event) => setCityId(event.target.value)}>{cities.map((city) => <option key={city.id} value={city.id}>{city.name}</option>)}</select></label><div className="booking-place-search"><input value={keywords} onChange={(event) => setKeywords(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void search(); } }} placeholder={currentPlace ? currentPlace.name : "输入酒店 / 机场 / 车站"} aria-label="搜索绑定地点"/><button type="button" onClick={() => void search()} disabled={loading}>{loading ? "搜索中…" : "搜索"}</button></div>{results.length ? <ul className="booking-place-results">{results.map((result) => <li key={result.id}><div><b>{result.name}</b><small>{[result.district, result.address].filter(Boolean).join(" · ") || "暂无地址"}</small></div><button type="button" onClick={() => void bind(result.id)} disabled={loading}>绑定</button></li>)}</ul> : null}{error ? <p className="form-error" role="alert">{error}</p> : null}<small>未找到时可在“地图”中选点；绑定不会改变订单日期、金额或费用分摊。</small></div></details>;
}
