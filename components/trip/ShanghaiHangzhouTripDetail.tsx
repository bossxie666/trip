...
import { ShanghaiHangzhouMapDesk } from "@/components/trip/ShanghaiHangzhouMapDesk";
...
const eastTrains: Train[] = [
  ...
  { id: "g7553", no: "G7553", leave: "21:09", arrive: "22:04", fare: 86, duration: "55 分", metro: true, note: "候选：外滩留时更足，仍有地铁余量" },
...
const southTrains: Train[] = [
  { id: "d57", no: "D57", leave: "19:40", arrive: "21:32", fare: 39, duration: "1 小时 52 分", metro: true, note: "省钱候选：到站后可接 5 号线" },
...
const hotelCards = [
  ...
  { name: "里外里假日酒店一类接驳酒店", zone: "disney", price: "¥280–420", why: "常见免费班车，适合上海阶段同行人", risk: "接驳班次必须逐家确认" },
...
export default function ShanghaiHangzhouTripDetail({ mapWorkspace }: { mapWorkspace?: PlaceWorkspace | null }) {
  ...
  const [arrival, setArrival] = useState<"east" | "south">("east"); const [trainId, setTrainId] = useState<string | null>(null);
  const [hzMode, setHzMode] = useState<"metro" | "taxi">("metro"); const [mapCity, setMapCity] = useState<"shanghai" | "hangzhou">("shanghai");
  const [mapKind, setMapKind] = useState<"amap" | "detail">("amap"); const [disneyRoute, setDisneyRoute] = useState("balanced");
  ...
  const trains = arrival === "east" ? eastTrains : southTrains; const selectedTrain = trainId ? trains.find(x => x.id === trainId) ?? null : null;
  const railFare = selectedTrain?.fare ?? (arrival === "east" ? 86 : 39);
  const effectiveHzMode = selectedTrain?.metro === false ? "taxi" : hzMode;
  ...
  const sharedEach = useMemo(() => hotelPrice / 4 + selectedLuggage.total / 4 + nightFare / 4 + selectedZone.dayTransit / 4 + selectedRiver.total / 4 + selectedZone.toTrain / 4 + railFare + hzFare + meals, [hotelPrice, selectedLuggage, nightFare, selectedZone, selectedRiver, railFare, hzFare, meals]);
  ...
  const changeArrival = (next: "east" | "south") => { setArrival(next); setTrainId(null); setHzMode("metro"); };
  const pins = mapCity === "shanghai" ? shanghaiPins : hangzhouPins; const activeDisney = disneyRoutes.find(x => x.id === disneyRoute)!;
  const activeMapStage = mapWorkspace?.stages.find(stage => stage.city?.name === (mapCity === "shanghai" ? "上海" : "杭州"));
  const showAmap = mapKind === "amap" && Boolean(mapWorkspace);
  ...
  const nightDetail = night === "metro" ? selectedZone.nightRoute : `打车：迪士尼→${selectedZone.name}，上海阶段4人约 ${money(selectedZone.nightTaxi)}`;
  ...
  return <main>
    ...
    <section className="hero" id="top">
      ...
      <div className="hero-gallery" aria-label="上海杭州旅行画廊"><figure className="gallery-card card-airport"><div className="scene scene-airport"><span>PVG</span></div><figcaption>09:10 · 浦东落地</figcaption></figure><figure className="gallery-card card-disney"><div className="scene scene-disney"><span>DAY 1</span></div><figcaption>上海迪士尼 · 四人会合</figcaption></figure><figure className="gallery-card card-river"><div className="scene scene-river"><span>DAY 2</span></div><figcaption>外滩 ↔ 陆家嘴</figcaption></figure><div className="route-ticket"><b>SZX — SHA — HGH</b><span>2 DAYS · SHANGHAI 4 · HANGZHOU 5</span></div><i className="tape tape-a"/><i className="tape tape-b"/></div>
    ...
    <section className="planner" id="planner"><header className="section-heading"><span>01 / DECISION DESK</span><h2>你来选，我把后果算出来</h2><p>每次点击都会同时改变路线、时间风险和各阶段参与成员的明细。页面不显示整组总价，只显示个人承担。</p></header>
    ...
    <section className="people-costs"><header className="section-heading compact"><span>02 / PERSONAL COST</span><h2>我的个人消费</h2><p>只保留我自己的完整明细。同行人的来沪方式和个人消费不同，放到右侧计算器由她们自己填写。</p></header>
      <div className="personal-cost-layout">
        <article className="my-cost-card"><header><div><span>深圳出发 · 机场两人组</span><h3>我的已知费用</h3></div><strong>{money(myTotal)}</strong></header><div className="cost-lines"><CostLine name="深圳→上海机票" amount={560} note="截图确认约 ¥560"/><CostLine name="迪士尼门票" amount={366} note="已购买"/><CostLine name="浦东→迪士尼" amount={airportEach} note="只在机场落地的两人之间平分"/><CostLine name="上海住宿" amount={hotelPrice / 4} note={`总房费 ${money(hotelPrice)} ÷ 4`}/><CostLine name="散场→酒店" amount={nightFare / 4} note="四人平分"/><CostLine name="行李寄存" amount={selectedLuggage.total / 4} note="按四人平分"/><CostLine name="Day 2 市内交通" amount={(selectedZone.dayTransit + selectedRiver.total + selectedZone.toTrain) / 4} note="酒店→陆家嘴＋过江＋上海南"/><CostLine name={selectedTrain ? `${selectedTrain.no} 候选车次` : "上海→杭州铁路候选预算"} amount={railFare} note={selectedTrain ? `${arrival === "east" ? "杭州东" : "杭州南"}到站 · 当前比较项，未确认` : "参考估算，未确认具体车次"}/><CostLine name="杭州到酒店" amount={hzFare} note={`${effectiveHzMode === "metro" ? "地铁" : "打车平分"}到杭州站附近`}/><CostLine name="两天吃喝预留" amount={meals} note="跟随上方滑杆"/></div></article>
