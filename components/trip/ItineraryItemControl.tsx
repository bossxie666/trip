...
type Item = { id: string; title: string; note: string | null; startTimeLocal: string | null; endTimeLocal?: string | null; timeMode?: "untimed" | "start_only" | "range" | "all_day" | "opening_hours"; openingHoursNote?: string | null; durationMinutes: number | null; dayId: string };
...
export function ItineraryItemControl({ slug, item, days, members = [], participantStates = {}, participantOverrides = {}, dayPresenceState = {} }: { slug: string; item: Item; days: { id: string; label: string }[]; members?: Member[]; participantStates?: Record<string, PresenceState>; participantOverrides?: Record<string, "included" | "excluded">; dayPresenceState?: Record<string, PresenceState> }) {
  ...
  async function save() {
    ...
    try {
      const response = await fetch(endpoint, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ title, note: note || null, startTimeLocal: startTimeLocal || null, endTimeLocal: endTimeLocal || null, timeMode, openingHoursNote: openingHoursNote || null, durationMinutes: duration ? Number(duration) : null, dayId, ...(participantDirty ? { participantMemberIds: participantMode === "inherit" ? null : selectedMemberIds } : {}) }) });
