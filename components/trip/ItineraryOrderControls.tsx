...
export function ItineraryOrderControls({ slug, dayId, itemId, itemIds, locked }: { slug: string; dayId: string; itemId: string; itemIds: string[]; locked: boolean }) {
  ...
  async function move(delta: number) {
    ...
    try { const response = await fetch(`/api/trips/${encodeURIComponent(slug)}/plan/items`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ dayId, orderedItemIds: next }) }); if (!response.ok) throw new Error("排序失败"); refreshWorkspace(); } catch { setBusy(false); }
  }
