...
function BookingEditControl({ slug, booking, members }: { slug: string; booking: PersonalBudgetWorkspace["bookings"][number]; members: Member[] }) {
  ...
  async function save() {
    ...
    try {
      const costLineAllocations = booking.costLines.map((line) => allocationModes[line.id] === "equal" ? { costLineId: line.id, allocationMode: "equal", memberIds: allocationMembers[line.id] || [] } : { costLineId: line.id, allocationMode: "custom", allocations: (allocationMembers[line.id] || []).map((memberId) => ({ memberId, amountMinor: toMinor(allocationAmounts[line.id]?.[memberId] || "0") })) });
      ...
      setOpen(false); refreshWorkspace();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "订单更新失败"); }
    finally { setSaving(false); }
  }
  return <details className="budget-booking-edit" open={open} onToggle={(event) => { const nextOpen = (event.currentTarget as HTMLDetailsElement).open; setOpen(nextOpen); if (nextOpen) setSaving(false); }}><summary>编辑订单信息</summary><div><label>名称<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>状态<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="tentative">待确认</option><option value="confirmed">已确认</option><option value="cancelled">已取消</option></select></label><label>订单总额（元）<input inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></label><fieldset><legend>订单使用成员</legend><div className="budget-member-checks">{members.map((member) => <label key={member.id}><input type="checkbox" checked={participantIds.includes(member.id)} onChange={() => toggleParticipant(member.id)} /> {member.displayName}</label>)}</div></fieldset>{booking.costLines.map((line) => { const allocationMode = allocationModes[line.id] || line.allocationMode; return <fieldset key={line.id}><legend>{line.title} · {allocationMode === "equal" ? "平均分摊" : "自定义分摊"}（{money(line.amountMinor)}）</legend><div className="budget-split-mode"><label><input type="radio" name={`booking-split-${line.id}`} checked={allocationMode === "equal"} onChange={() => setAllocationModes((current) => ({ ...current, [line.id]: "equal" }))} /> 平均分摊</label><label><input type="radio" name={`booking-split-${line.id}`} checked={allocationMode === "custom"} onChange={() => setAllocationModes((current) => ({ ...current, [line.id]: "custom" }))} /> 自定义金额</label></div><div className="budget-member-checks">{members.map((member) => <label key={member.id}><input type="checkbox" checked={(allocationMembers[line.id] || []).includes(member.id)} onChange={() => toggleAllocationMember(line.id, member.id)} /> {member.displayName}</label>)}</div>{allocationMode === "custom" && <div className="budget-custom-split">{(allocationMembers[line.id] || []).map((memberId) => <label key={memberId}>{members.find((member) => member.id === memberId)?.displayName}<input inputMode="decimal" value={allocationAmounts[line.id]?.[memberId] || ""} onChange={(event) => setAllocationAmounts((current) => ({ ...current, [line.id]: { ...(current[line.id] || {}), [memberId]: event.target.value } }))} placeholder="0.00" /></label>)}</div>}</fieldset>; })}<button type="button" onClick={save} disabled={saving}>{saving ? "保存中…" : "保存订单"}</button><ErrorMessage value={error} /></div></details>;
}
...
export function BudgetWorkspace({ slug, budget, members, days, cities = [], routeSegmentsByDay, routePreferences, costMode = "expected" }: { slug: string; budget: PersonalBudgetWorkspace | null; members: Member[]; days: Day[]; cities?: { id: string; name: string }[]; routeSegmentsByDay?: Record<string, RouteSegment[]>; routePreferences?: RoutePreference[]; costMode?: "expected" | "actual" }) {
  ...
  async function savePlan(category: BudgetCategory) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || "保存失败");
      refreshWorkspace();
    } catch (caught) { setPlanError(caught instanceof Error ? caught.message : "保存失败"); }
