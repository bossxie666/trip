...
function BookingEditControl({ slug, booking }: { slug: string; booking: PersonalBudgetWorkspace["bookings"][number] }) {
  const [open, setOpen] = useState(false), [title, setTitle] = useState(booking.title), [status, setStatus] = useState(booking.status), [amount, setAmount] = useState(yuan(booking.totalAmountMinor)), [saving, setSaving] = useState(false), [error, setError] = useState("");
  async function save() {
    setSaving(true); setError("");
    try {
      const response = await fetch(`/api/trips/${encodeURIComponent(slug)}/budget/bookings/${encodeURIComponent(booking.id)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ title, status, totalAmountMinor: amount ? toMinor(amount) : null }) });
...
export function BudgetWorkspace({ slug, budget, members, days, routeSegmentsByDay, routePreferences }: { slug: string; budget: PersonalBudgetWorkspace | null; members: Member[]; days: Day[]; routeSegmentsByDay?: Record<string, RouteSegment[]>; routePreferences?: RoutePreference[] }) {
  ...
  return <section className="budget-view"><header className="workspace-view-heading"><div><span>BUDGET · {members.find((member) => member.id === budget.memberId)?.displayName || "我"}</span><h2>我的个人预算</h2><p className="budget-subtitle">固定费用、计划花费和已发生消费只统计当前成员。</p></div><button type="button" className="budget-primary budget-add-button" onClick={() => setShowExpenseForm(true)}>＋记一笔</button></header><div className="budget-summary-grid"><article><span>预计总额</span><strong>{money(totals.expectedMinor)}</strong><small>固定费用 + 计划花费</small></article><article><span>已确认 / 已发生</span><strong>{money(totals.confirmedMinor)}</strong><small>已分摊订单 + 实际消费</small></article><article><span>计划剩余</span><strong className={totals.remainingMinor < 0 ? "over" : ""}>{money(totals.remainingMinor)}</strong><small>计划花费 − 实际消费</small></article></div><EstimatedTransport slug={slug} days={days} segmentsByDay={routeSegmentsByDay} preferences={routePreferences} currentMemberId={budget.memberId}/><div className="budget-bookings"><header><h3>已确认订单 · 我的承担</h3><span>订单总价不会直接算入个人费用</span></header>{budget.bookings.map((booking) => <article key={booking.id}><div><b>{bookingIcon(booking.type)} {booking.title}</b><small>{booking.pending ? "我的费用待确认" : "已记录个人分摊"}</small></div><strong className={booking.pending ? "pending" : ""}>{booking.pending ? "我的费用待确认" : money(booking.ownAmountMinor)}</strong><BookingEditControl slug={slug} booking={booking} /></article>)}{!budget.bookings.length && <p className="budget-muted">暂无已确认订单。</p>}</div><div className="budget-plans"><header><h3>我的计划花费</h3><span>按类别设置个人预算</span></header>{categories.map((category) => <div className="budget-plan-row" key={category.key}><label>{category.label}<input inputMode="decimal" value={values[category.key]} onChange={(event) => setPlanValues((current) => ({ ...current, [category.key]: event.target.value }))} placeholder="0.00" /></label><button type="button" onClick={() => savePlan(category.key)} disabled={savingPlan === category.key}>{savingPlan === category.key ? "保存中…" : "保存"}</button></div>)}<ErrorMessage value={planError} /></div><div className="budget-expenses"><header><h3>最近消费</h3><span>只显示你有权查看的费用</span></header>{budget.expenses.map((expense) => <article key={expense.id}><div><b>{expense.title}</b><small>{categoryLabel[expense.category]} · {expense.scope === "shared" ? `共享 · ${expense.payerName ? `${expense.payerName}付款` : "付款人待定"}` : "个人"}</small></div><strong>{money(expense.ownAmountMinor)}</strong><ExpenseEditControl slug={slug} expense={expense} currentMemberId={budget.memberId} /></article>)}{!budget.expenses.length && <p className="budget-muted">还没有消费记录，点击“＋记一笔”开始。</p>}</div><p className="budget-disclaimer">固定费用只统计已有个人分摊；没有你的 BookingCostAllocation 时显示“我的费用待确认”。共享费用的付款人和承担人分开记录，个人页只计入你的 allocation。</p>{showExpenseForm && <ExpenseForm slug={slug} members={members} days={days} currentMemberId={budget.memberId} onClose={() => setShowExpenseForm(false)} />}</section>;
}
