...
export function GenericPlacePicker({ slug, existing = [], value, onChange }: { slug: string; existing?: GenericPlaceChoice[]; value: GenericPlaceChoice | null; onChange: (place: GenericPlaceChoice | null) => void; autoFocus?: boolean }) {
  ...
  const choices = query.trim() ? results : existing;
  return <div className="generic-place-picker">
    <input value={query} onChange={(event) => { setQuery(event.target.value); onChange(null); }} placeholder="搜索酒店、机场、车站、景点、餐厅…" aria-label="搜索真实地点" />
    {!query.trim() && existing.length > 0 && <small>最近使用 · 当前住宿 · 想去地点</small>}
