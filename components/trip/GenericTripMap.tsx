...
export function GenericTripMap({ slug, initial, cityId, fallbackCenter }: { slug: string; initial: MapWorkspace; cityId?: string; fallbackCenter?: readonly [number, number] }) {
  const container = useRef<HTMLDivElement>(null), mapRef = useRef<AMapObject | null>(null), overlays = useRef<unknown[]>([]), markerObjects = useRef<unknown[]>([]), lastFitSignature = useRef("");
  ...
  useEffect(() => { const map = mapRef.current; if (!mapReady || !map || !markerObjects.current.length || lastFitSignature.current === fitSignature) return; lastFitSignature.current = fitSignature; map.setFitView(markerObjects.current); }, [fitSignature, mapReady]);
  ...
  return <div className="generic-trip-map">
    ...
    <div className="route-modes">{(["walking", "transit", "driving", "taxi"] as AMapRouteMode[]).map((item) => <button type="button" className={mode === item ? "active" : ""} key={item} onClick={() => setMode(item)}>{modeLabels[item]}</button>)}<button type="button" className="route-submit" disabled={!selectedOriginId || !selectedDestinationId || selectedOriginId === selectedDestinationId || routing} onClick={plan}>{routing ? "规划中…" : "规划路线"}</button></div>
