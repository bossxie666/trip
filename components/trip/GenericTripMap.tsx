...
export function GenericTripMap({ slug, initial, cityId, fallbackCenter }: { slug: string; initial: MapWorkspace; cityId?: string; fallbackCenter?: readonly [number, number] }) {
  ...
  return <div className="generic-trip-map">
    ...
    <div className="route-modes">{(["transit", "taxi", "walking", "bicycling"] as AMapRouteMode[]).map((item) => <button type="button" className={mode === item ? "active" : ""} key={item} onClick={() => setMode(item)}>{modeLabels[item]}</button>)}<button type="button" className="route-submit" disabled={!selectedOriginId || !selectedDestinationId || selectedOriginId === selectedDestinationId || routing} onClick={plan}>{routing ? "规划中…" : "规划路线"}</button></div>
