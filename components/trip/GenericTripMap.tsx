...
export function GenericTripMap({ slug, initial, cityId }: { slug: string; initial: MapWorkspace; cityId?: string }) {
  ...
  const [workspace, setWorkspace] = useState(initial), [dayId, setDayId] = useState(initialDay?.id || ""), [originId, setOriginId] = useState(""), [destinationId, setDestinationId] = useState(""), [mode, setMode] = useState<AMapRouteMode>("walking"), [route, setRoute] = useState<AMapRouteResult | null>(null), [error, setError] = useState(""), [loading, setLoading] = useState(true), [routing, setRouting] = useState(false);
  ...
  useEffect(() => { const map = mapRef.current, AMap = window.AMap; if (!map || !AMap) return; if (overlays.current.length) map.remove(overlays.current); const markers = allPlaces.map((place) => new AMap.Marker({ position: [place.longitude!, place.latitude!], title: place.name, opacity: place.planStatus === "candidate" ? 0.48 : 1, label: { content: `${place.markerNumber}. ${place.name}`, direction: "top" } })); const lines = (route?.polylines || []).map((path) => new AMap.Polyline({ path, strokeColor: "#bf6648", strokeWeight: 6, strokeOpacity: 0.9, showDir: true })); overlays.current = [...markers, ...lines]; if (overlays.current.length) { map.add(overlays.current); map.setFitView(overlays.current); } }, [allPlaces, route]);
