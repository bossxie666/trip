...
type ExistingTransport = { id: string; title: string; origin: GenericPlaceChoice | null; destination: GenericPlaceChoice | null; type: string; totalAmountMinor: number | null };
...
export function PlanAddControl({ slug, days, defaultDayId, currentMemberId, existingTransport = [], members = [], existingPlaces = [] }: { slug: string; days: DayOption[]; defaultDayId: string; currentMemberId?: string | null; existingTransport?: ExistingTransport[]; members?: MemberOption[]; existingPlaces?: GenericPlaceChoice[] }) {
  ...
  const [origin, setOrigin] = useState<GenericPlaceChoice | null>(null), [destination, setDestination] = useState<GenericPlaceChoice | null>(null);
  ...
  function reset() { setPlace(null); setTitle(""); setNote(""); setOrigin(null); setDestination(null); setStartDate(""); setEndDate(""); setStartTime(""); setEndTime(""); setReference(""); setBookingCode(""); setProvider(""); setFare(""); setTimeMode("untimed"); setReuseId(""); setParticipantMemberIds(currentMemberId ? [currentMemberId] : []); setError(""); }
  function reuseTransport(id: string) { setReuseId(id); const existing = existingTransport.find((item) => item.id === id); if (!existing) return; setOrigin(existing.origin); setDestination(existing.destination); setTransportType(existing.type === "flight" ? "flight" : existing.type === "train" ? "railway" : "ferry"); setFare(existing.totalAmountMinor == null ? "" : String(existing.totalAmountMinor / 100)); }
