...
const hotelZones = [
  { id: "south", name: "上海南站", tag: "第二天赶车最稳", nightMetro: 28, nightTaxi: 130, nightTime: "82 / 58 分", dayTransit: 20, dayTime: "45 分", toTrain: 20, toTrainTime: "52 分" },
  { id: "xujiahui", name: "徐家汇", tag: "通勤最均衡", nightMetro: 28, nightTaxi: 115, nightTime: "68 / 50 分", dayTransit: 16, dayTime: "36 分", toTrain: 12, toTrainTime: "50 分" },
  { id: "bund", name: "外滩周边", tag: "夜景方便、房价偏高", nightMetro: 24, nightTaxi: 108, nightTime: "75 / 48 分", dayTransit: 12, dayTime: "22 分", toTrain: 20, toTrainTime: "40 分" },
  { id: "luz", name: "陆家嘴", tag: "东方明珠就在楼下", nightMetro: 24, nightTaxi: 92, nightTime: "64 / 42 分", dayTransit: 0, dayTime: "步行 10 分", toTrain: 24, toTrainTime: "60 分" },
  { id: "disney", name: "迪士尼周边", tag: "散场最快、第二天最远", nightMetro: 0, nightTaxi: 32, nightTime: "接驳 22 / 18 分", dayTransit: 24, dayTime: "62 分", toTrain: 20, toTrainTime: "45 分＋寄存" },
];
...
export default function Home() {
  ...
  const people = [
    { name: "旅伴 A", role: "深圳出发 · 机场两人组", inbound: 560, airport: airportEach }, { name: "旅伴 B", role: "深圳出发 · 机场两人组", inbound: 560, airport: airportEach },
    { name: "旅伴 C", role: "迪士尼会合", inbound: 0, airport: 0 }, { name: "旅伴 D", role: "迪士尼会合", inbound: 0, airport: 0 },
  ];
  ...
  return <main>
    ...
    <section className="people-costs"><header className="section-heading compact"><span>02 / PER PERSON</span><h2>每个人分别花多少</h2><p>迪士尼票 ¥366/人已买，仍计入旅行成本。C、D 的来沪交通未知，因此明确留空，不拿 A、B 的机票替他们计算。</p></header><div className="people-grid">{people.map(person => {
      const total = sharedEach + 366 + person.inbound + person.airport;
      return <details className="person-card" key={person.name} open={person.name === "旅伴 A"}><summary><div><span>{person.role}</span><h3>{person.name}</h3></div><strong>{money(total)}</strong></summary><div className="cost-lines"><CostLine name="深圳→上海机票" amount={person.inbound} note={person.inbound ? "截图确认约 ¥560" : "未提供，不计入"}/><CostLine name="迪士尼门票" amount={366} note="已购买"/><CostLine name="浦东→迪士尼" amount={person.airport} note={person.airport ? "只在 A、B 之间平分" : "在园区会合，不承担"}/><CostLine name="上海住宿" amount={hotelPrice / 4} note={`总房费 ${money(hotelPrice)} ÷ 4`}/><CostLine name="散场→酒店" amount={nightFare / 4} note="四人平分"/><CostLine name="行李寄存" amount={selectedLuggage.total / 4} note="按四人平分"/><CostLine name="Day 2 市内交通" amount={(selectedZone.dayTransit + selectedRiver.total + selectedZone.toTrain) / 4} note="酒店→陆家嘴＋过江＋上海南"/><CostLine name={`${selectedTrain.no} 火车票`} amount={selectedTrain.fare} note={`${arrival === "east" ? "杭州东" : "杭州南"}到站`}/><CostLine name="杭州到酒店" amount={hzFare} note={`${effectiveHzMode === "metro" ? "地铁" : "打车平分"}到杭州站附近`}/><CostLine name="两天吃喝预留" amount={meals} note="可用滑杆调整"/></div></details>;
    })}</div><p className="cost-footnote">以上是做选择用的预算模型，不是付款报价；网约车、酒店、寄存和餐饮会浮动。网站不会把未知费用硬猜进数字。</p></section>
