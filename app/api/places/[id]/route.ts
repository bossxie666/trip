...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const code = error instanceof Error ? error.message : ""; const status = code === "PLACE_NOT_FOUND" ? 404 : code === "PLACE_DUPLICATE" ? 409 : code === "PLACE_NOT_MANUAL" ? 403 : 500;
    return Response.json({ error: code === "PLACE_NOT_FOUND" ? "地点不存在。" : code === "PLACE_DUPLICATE" ? "同一城市已有同名地点。" : code === "PLACE_NOT_MANUAL" ? "只有手工地点可以修改。" : "修改地点失败。" }, { status });
  }
}
