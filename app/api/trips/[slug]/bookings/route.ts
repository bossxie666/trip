...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const body = await request.json() as { type?: BookingType; status?: BookingStatus; title?: string; provider?: string | null; startDateLocal?: string | null; endDateLocal?: string | null; startAt?: string | null; endAt?: string | null; place?: PlaceInput; origin?: PlaceInput; destination?: PlaceInput; totalAmountMinor?: number | null; participantMemberIds?: string[]; bookingReference?: string | null; notes?: string | null };
    const db = getDb();
    ...
    const isTransport = isTransportBooking({ type, originPlaceId, destinationPlaceId }) || type === "flight" || type === "train";
    if (isTransport && (!originPlaceId || !destinationPlaceId)) return Response.json({ error: "班次 / 票务交通必须选择真实出发地与到达地。" }, { status: 400 });
    const booking = await createBooking({ tripId: trip.id, type, status, title: body.title?.trim() || (type === "hotel" ? "住宿安排" : "交通安排"), provider: body.provider?.trim() || null, temporalKind: type === "hotel" ? "date_range" : (body.endAt ? "interval" : "instant"), startAt: body.startAt || null, endAt: body.endAt || null, startDateLocal: body.startDateLocal || null, endDateLocal: body.endDateLocal || null, timezone: trip.timezone || "Asia/Shanghai", placeId, originPlaceId, destinationPlaceId, originLabel: null, destinationLabel: null, totalAmountMinor: body.totalAmountMinor ?? null, currency: body.totalAmountMinor == null ? null : "CNY", bookingReference: body.bookingReference?.trim() || null, notes: body.notes?.trim() || null, participantMemberIds }, actor.id);
