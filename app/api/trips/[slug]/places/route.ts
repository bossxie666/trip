...
import { addPlaceToDay, createManualPlace, getPlaceWorkspace, removePlaceFromDay, removePlaceFromTrip, reorderDayPlaces } from "@/services/place-repository.server";

function errorResponse(error: unknown) {
  const code = error instanceof Error ? error.message : "UNKNOWN";
  const messages: Record<string, [string, number]> = {
    TRIP_NOT_FOUND: ["没有找到这条行程。", 404], DAY_NOT_IN_TRIP: ["这个 Day 不属于当前行程。", 400], PLACE_NOT_FOUND: ["地点不存在。", 404], CITY_NOT_IN_TRIP: ["城市不属于当前行程。", 400], PLACE_CITY_NOT_IN_TRIP: ["该地点不属于当前行程的城市。", 400], INVALID_ORDER: ["地点顺序与当前 Day 不一致。", 409], PLACE_STILL_IN_DAY: ["地点仍在日程中，请先从 Day 移除。", 409],
  };
  const [message, status] = messages[code] || ["地点操作失败，请重试。", 500]; return Response.json({ error: message }, { status });
}
...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const slug = (await params).slug; const body = await request.json() as { action?: string; dayId?: string; placeId?: string; name?: string; cityId?: string; address?: string };
