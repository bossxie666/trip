...
import { dayRecords, memberPresenceWindowRecords, memberRecords, tripMemberRecords, tripRecords } from "@/db/schema";
import { getCurrentMember } from "@/services/auth.server";
import { replaceDayPresence } from "@/services/presence-repository.server";
...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const windows = await db.select().from(memberPresenceWindowRecords).where(eq(memberPresenceWindowRecords.tripId, trip.id));
  ...
  return Response.json({ day: { id: day.id, title: day.title, date: day.date }, members, states });
}
...
export async function PUT(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (!actor) return Response.json({ error: "请先验证旅行成员身份。" }, { status: 401 });
    const { slug } = await params;
    const body = await request.json() as { dayId?: string; memberIds?: string[] };
    if (!body.dayId || !Array.isArray(body.memberIds)) return Response.json({ error: "缺少当天成员数据。" }, { status: 400 });
    const db = getDb();
    const trip = (await db.select({ id: tripRecords.id }).from(tripRecords).where(eq(tripRecords.slug, slug)).limit(1))[0];
    if (!trip) return Response.json({ error: "行程不存在。" }, { status: 404 });
    ...
    const presence = await replaceDayPresence({ tripId: trip.id, dayId: body.dayId, memberIds: [...new Set(body.memberIds.map(String))], actorMemberId: actor.id });
    return Response.json({ presence });
  } catch (error) {
    const code = error instanceof Error ? error.message : "";
    const status = code === "MEMBER_NOT_IN_TRIP" ? 403 : code === "DAY_CONTEXT_INCOMPLETE" || code === "PRESENCE_OVERLAP" ? 409 : 400;
    return Response.json({ error: status === 403 ? "你不是这条行程的成员。" : status === 409 ? "当天已有不完整的在场区间，请先整理后再确认。" : "当天成员数据无效。" }, { status });
  }
}
