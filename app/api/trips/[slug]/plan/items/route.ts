...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const body = await request.json() as { recommendationId?: string; dayId?: string; placeId?: string; providerPlaceId?: string; cityId?: string; title?: string; itemType?: "place" | "meal" | "transit" | "lodging" | "activity" | "note"; note?: string | null; startTimeLocal?: string | null; durationMinutes?: number | null };
    const db = getDb();
