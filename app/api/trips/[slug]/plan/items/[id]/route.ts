...
import { dayRecords, itineraryItemParticipantOverrideRecords, memberPresenceWindowRecords, memberRecords, tripMemberRecords, tripRecords } from "@/db/schema";
...
export async function GET(_request: Request, { params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  const [members, overrides, day, trip, presenceRows, windows] = await Promise.all([
...
export async function PATCH(request: Request, { params }: { params: Promise<{ slug: string; id: string }> }) {
  try {
    ...
    const body = await request.json() as { title?: string; note?: string | null; startTimeLocal?: string | null; durationMinutes?: number | null; dayId?: string; participantMemberIds?: string[] | null };
